...
def _parse_filename(filename, class_label):
    """
    Parse CNVVE filename to extract metadata.
    Format: {class}_{actor_id}_{additional_info}.wav
    """
    ...
    actor_id = parts[1]  # Second part after first underscore
...
def _create_readme(samples_path):
    with open(f"{samples_path}README.md", "w") as f:
        f.write("---\n")
        f.write("configs:\n")
        f.write("- config_name: default-cnvve\n")
        f.write("  data_files:\n")
        f.write("  - split: train\n")
...
def prepare_cnvve(data_path="data/cnvve/", train_size=0.8, test_size=0.5, min_samples_per_class=10):
    """
    Prepare the CNVVE dataset. Creates train, val, test splits and saves them into the dataset folder.

    args:
        - data_path: str, path to the CNVVE dataset
        - train_size: float, size of the train set
        - test_size: float, size of the test set
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare CNVVE dataset")
    parser.add_argument(
        "--data-path",
        type=str,
        default="data/cnvve/",
        help="Path to the CNVVE dataset (default: data/cnvve/)"
    )
    parser.add_argument(
        "--train-size", 
        type=float, 
