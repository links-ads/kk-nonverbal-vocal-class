...
def _parse_filename(filename, class_label):
    """
    Parse nonverbal vocalization filename to extract metadata.
    Format: {speaker ID}_{class}_{index}_{sex}_{age}_{location}_{quality}_{noise}.wav
    """
    ...
    file_class = parts[1]
    index = parts[2]
    sex = parts[3]  # 0 for male, 1 for female
    age = parts[4]
    location = parts[5]
    quality = parts[6]
    noise = parts[7]
...
def _drop_columns(df):
    ...
    """
    df = df.drop(columns=['speaker_id', 'sex', 'age', 'location', 'quality', 'noise'])
...
def prepare_nonverbal_vocalization(data_path="data/nonverbal_vocalization_dataset/NonverbalVocalization/", train_size=0.8, test_size=0.5, min_samples_per_class=10):
    ...
    args:
        - data_path: str, path to the nonverbal vocalization dataset
        - train_size: float, size of the train set
        - test_size: float, size of the test set
    ...
    # Clean up existing samples folder to avoid conflicts
    if os.path.exists(samples_path):
        print(f"Removing existing samples folder: {samples_path}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Prepare nonverbal vocalization dataset")
    parser.add_argument(
        "--data-path",
        type=str,
    ...
    print(f"Preparing nonverbal vocalization dataset at {args.data_path}")
    print(f"Parameters: train_size={args.train_size}, test_size={args.test_size}, min_samples={args.min_samples}")
    
    samples_path = prepare_nonverbal_vocalization(
