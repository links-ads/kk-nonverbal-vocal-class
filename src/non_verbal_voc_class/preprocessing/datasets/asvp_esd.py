...
def _create_subsamples(data_path, new_folder):
    ...
    for actor_folder in os.listdir(data_path):
        ...
        for file in os.listdir(actor_path):
            ...
            file_info = _parse_filename(file)
            if not file_info:
                continue
                
            # Filter only non-speech audios (vocal channel = 02)
            if file_info['vocal_channel'] != 'non_speech':
                continue
            ...
            file_metadata = {
                ...
                'label': file_info['emotion'],
                'gender': file_info['gender'],
                'vocal_channel': file_info['vocal_channel']
            }
...
def _drop_columns(df, columns_to_keep=None):
    """
    Keep only specified columns in the DataFrame.
    """
    if columns_to_keep is None:
        columns_to_keep = ['file_name', 'label']
...
def prepare_asvp_esd(data_path="data/asvp-esd/Audio/", train_size=0.8, test_size=0.5, min_samples_per_class=10):
    ...
    parent_dir = os.path.dirname(os.path.dirname(data_path))  # Go up two levels from Audio folder
    samples_path = f"{parent_dir}/samples/"
