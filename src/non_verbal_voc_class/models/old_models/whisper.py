...
class WhisperWrapper(nn.Module):
    def __init__(
        ...
        assert config.model_type == 'whisper', "model type must be whisper"
        ...
        self.max_raw_length = torch.tensor(config.max_duration * self.config.sampling_rate) # (e.g., 4 * 16000 = 64000)
        ...
        # take only the first 200 elements of embed_positions to match the max_feat_length
        self.backbone_model.encoder.embed_positions.weight = nn.Parameter(self.backbone_model.encoder.embed_positions.weight[:self.max_feat_length])
        self.backbone_model.encoder.embed_positions.weight.requires_grad = False
    ...
    def forward(self,
        ...
        """
        args:
        - features: Tensor, preprocessed (AutoFeatureExtractor) input features (batch_size, n_mels, n_feature_frames)
        ...
        # 1. encode input length to feature length
        if length is not None:
            length = self.get_feat_extract_output_lengths(length.detach().cpu())
        # print(self.max_feat_length)
        # if length.max() > self.max_feat_length: max_len = self.max_feat_length
        # else: max_len = length.max()
        # self.backbone_model.encoder.embed_positions = self.backbone_model.encoder.embed_positions.from_pretrained(self.embed_positions[:self.max_feat_length])

        # 2. Replace positional emebddings
        # self.backbone_model.encoder.embed_positions = self.backbone_model.encoder.embed_positions.from_pretrained(self.embed_positions[:self.max_feat_length])
        ...
        return {'encoder_hidden_states': features, 'length': length}
