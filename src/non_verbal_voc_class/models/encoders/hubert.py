"""
Modified HubertEncoderLayer with support for adapter, LoRA, and embedding prompt fine-tuning methods.
Adapted from Wav2Vec2 implementation with SUPERB and IPET inspiration.
"""
import torch
...
class HubertEncoderLayer(nn.Module):
    def __init__(self, config):
        super().__init__()
        self.attention = hubert.HUBERT_ATTENTION_CLASSES[config._attn_implementation](
        ...
        if self.config.finetune_method == "embedding_prompt" or self.config.finetune_method == "combined":
            self.embed_prompt = nn.Parameter(torch.randn([1, self.config.embedding_prompt_dim, config.hidden_size]))
            nn.init.xavier_uniform_(self.embed_prompt)
        
        if (self.config.finetune_method == "lora" or 
            self.config.finetune_method == "combined"):
        ...
        if (self.config.finetune_method == "adapter" or 
            self.config.finetune_method == "adapter_l" or 
            self.config.finetune_method == "combined"):
    ...
    def forward(self, hidden_states, attention_mask=None, output_attentions=False):
        ...
        elif self.config.finetune_method == "adapter_l" or self.config.finetune_method == "combined":
